...
const GET_ANIME_YEAR = "GET_ANIME_YEAR";
...
const reducer = (state, action) => {
    switch(action.type){
        ...
        case GET_ANIME_YEAR:
            return{...state,animeYear:action.payload, loading:false}
        default:
            return state;
    }
}
...
export const GlobalContextProvider = ({children}) => {
    ...
    //     const data = await response.json();
    //     dispatch({type: GET_RANDOM_ANIME, payload: data.data})
    // }
    ...
    const getAnimeYear = async () => {
        dispatch({type: LOADING})
        const response = await fetch(`https://api.jikan.moe/v4/random/anime`);
        const data = await response.json();
        dispatch({type: GET_ANIME_YEAR, payload: data.data})
    }
    //initial render
    React.useEffect(() => {
        getPopularAnime();
